package minecraft2d.view;

import minecraft2d.memento.Caretaker;
import minecraft2d.memento.ConcreteMemento;
import minecraft2d.memento.Memento;
...
public class Janela extends JFrame implements KeyListener{
    ...
    public Janela() {
        ...
        caretaker.saveState(save_state());
    }
    
    public Memento save_state() {
        return new ConcreteMemento(blocos.clone(),posAv,pose_avatar,bloco_direita);
    }
    
    public void load_state(Memento memento) {
        int[] tela = memento.getTela();
        int PosAv = memento.getPosAv();
        int PoseAv = memento.getPoseAv();
        boolean OlhandoDireita = memento.getOlhandoDireita();
    ...
    public void keyPressed(KeyEvent key) {
     ...
     if(key.getKeyCode() == KeyEvent.VK_R){
         Memento memento = caretaker.ResetMundo();
         load_state(memento);
     }   
     
    //Memento do estado atual
    if (key.getKeyCode() == KeyEvent.VK_S) {
        caretaker.saveState(save_state());
    }
    
    //Restaura o estado salvo feito pelo último memento
    if (key.getKeyCode() == KeyEvent.VK_BACK_SPACE) {
        Memento memento = caretaker.loadState();
        if (memento != null) {
            load_state(memento);
        }
    }
